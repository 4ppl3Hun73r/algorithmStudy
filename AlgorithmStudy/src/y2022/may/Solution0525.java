...
public class Solution0525 {
    ...
    public int maxEnvelopes(int[][] envelopes) {
        ...
        int[] lis =new int[envelopes.length + 1]; // longest increasing subsequence
        lis[0] = Integer.MIN_VALUE;
        int ans = 0;
        for(int i = 0; i < envelopes.length; i++){
            int height = envelopes[i][1];
            int insertIndex = Arrays.binarySearch(lis, 0, ans, height);
            if (insertIndex < 0) {
                insertIndex = -(insertIndex + 1);
            }
            lis[insertIndex] = height;
            if (insertIndex == ans) {
                ans++;
            }
        }
    ...
    */
    public int maxEnvelopesDp(int[][] envelopes) {
        ...
        러시안 인형 처럼 작은게 큰 봉투에 들어가는 형식으로 최대 몇개까지 넣을 수 있는가?
        ...
        for (int i = 0; i < envelopes.length; i++) {
            int maxCnt = getMaxCnt(envelopes, i);
    ...
    private int getMaxCnt(int[][] envelopes, int idx) {
        if (cache.containsKey(idx)) {
            return cache.get(idx);
        }

        int max = 1;
        int[] before = envelopes[idx];
        for (int i = idx + 1; i < envelopes.length; i++) {
            int[] current = envelopes[i];
            if (before[0] < current[0] && before[1] < current[1]) {
                max = Math.max(max, getMaxCnt(envelopes, i) + 1);
            }
        }
        cache.put(idx, max);
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(s.maxEnvelopes(new int[][]{
                {1,1}, {1,2}
        }));
